...
const crearPuntoDeVenta = async (req, res) => {
    try {
      ...
      if (!["fisico", "online"].includes(tipo)) {
        return res.status(400).json({ message: "El tipo debe ser 'fisico' u 'online'" });
      }
  
      // Crear el punto de venta
      const nuevoPuntoVenta = await PuntoVenta.create({ nombre, tipo, direccion });
      res.status(201).json(nuevoPuntoVenta);
    } catch (error) {
      console.error("Error al crear el punto de venta:", error);
      res.status(500).json({ message: "Error al crear el punto de venta", error: error.message || error });
    }
  };
...
const obtenerPuntoDeVentaPorId = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Error al obtener el punto de venta", error: error.message || error });
  }
};
...
const obtenerPuntosDeVentaPages = async (req, res) => {
    try {
      ...
      console.error("Error al obtener los puntos de venta:", error);
      res.status(400).json({ message: "Error al obtener los puntos de venta: " + error.message });
    }
  };
...
const actualizarPuntoDeVenta = async (req, res) => {
  try {
    ...
    if (tipo && !["fisico", "online"].includes(tipo)) {
      return res.status(400).json({ message: "El tipo debe ser 'fisico' o 'online'" });
    }
    ...
    res.status(500).json({ message: "Error al actualizar el punto de venta", error: error.message || error });
  }
};
...
const eliminarPuntoDeVenta = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Error al eliminar el punto de venta", error: error.message || error });
  }
};
